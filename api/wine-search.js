...
function grapesOf(row){
  ...
  return grapes.map(g=>{const name=text(g.grapeDesc||g.grapeName||g.name||g.grape);const pct=number(g.grapePct||g.percentage);return name+(pct?` ${pct}%`:'')}).filter(Boolean).join(', ');
}
...
function mapProduct(row){
  ...
  const producer=text(at(row,'basic.producerName','producer.producerName','producer.producerShortName','producer.name','manufacturer.manufacturerName','manufacturer.name','producerName','producer'));
  const country=text(at(row,'origins.origin.country','origins.production.country','origin.country','production.country','country'));
  const region=text(at(row,'origins.origin.region','origins.origin.subRegion','origins.production.region','origin.region','origin.subRegion','region','district'));
  const vintage=text(at(row,'properties.vintage','basic.vintage','vintage','year')).replace(/^0$/,'');
  const rawType=text(at(row,'classification.mainProductTypeName','classification.productTypeName','classification.productType','basic.productType','productType','type'));
  const barcodeList=at(row,'logistics.barcodes');
  const barcode=text(Array.isArray(barcodeList)?barcodeList.map(x=>x.gtin||x.barcode).find(Boolean):at(row,'logistics.gtin','basic.gtin','gtin','barcode','ean'));
  const prices=at(row,'prices');
  const price=number(Array.isArray(prices)?(prices.find(x=>x.salesPrice)?.salesPrice||prices[0]?.salesPrice||prices[0]?.consumerPrice):at(row,'prices.salesPrice','prices.consumerPrice','price.salesPrice','salesPrice','price'));
