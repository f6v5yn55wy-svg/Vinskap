...
async function init(){
  ...
  $('tripForm').onsubmit=async e=>{e.preventDefault();const t={id:uid(),createdAt:Date.now(),name:$('tripName').value.trim(),text:$('tripText').value.trim(),image:tripPhotoFile?await fileToDataURL(tripPhotoFile):''};await put('trips',t);tripPhotoFile=null;closeModal('tripModal');render()};
