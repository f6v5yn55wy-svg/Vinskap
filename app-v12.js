...
function tripWineCard(w){
  return `<article class="trip-wine-card">${w.image?`<img src="${w.image}" alt="${safe(w.name)}">`:''}<div class="trip-wine-body"><h3>${safe(w.name)}${w.vintage?' '+safe(w.vintage):''}</h3>${w.text?`<p>${safe(w.text)}</p>`:''}<button class="delete" data-trip-wine-delete="${w.id}">Slett vin</button></div></article>`;
}
...
function resetWineForm(){
  $('wineForm').reset(); $('wineId').value=''; $('barcode').value=''; $('productId').value=''; $('labelImage').value=''; $('scanStatus').textContent='Ta bilde av etiketten.'; $('scanPreview').hidden=true; $('scanPreview').src=''; $('candidateList').innerHTML=''; $('labelCorrection').hidden=true; $('labelSearchText').value=''; $('quantity').value=1;
}
...
async function searchCorrectedLabel(){
  ...
  showStatus(`Søker etter «${query}» …`,true);
  $('candidateList').innerHTML='';
  try{
    const data=await searchServer({name:query});
    const results=data.results||[];
    setCandidates(results);
    if(!results.length){showStatus('Ingen treff. Kontroller stavemåten eller skriv både produsent og vintype.');return}
    const best=Number(results[0]?.score)||0;
    const second=Number(results[1]?.score)||0;
    if(best>=6 && (results.length===1 || best>second)){fillCandidate(results[0]);$('labelCorrection').hidden=true}
    else showStatus('Velg riktig vin fra forslagene under.');
...
async function handleLabel(file){
  ...
  try{
    ...
    if(best>=6){fillCandidate(results[0]);$('labelCorrection').hidden=true}
...
async function init(){
  ...
  $('addTripWineBtn').onclick=()=>{
    $('tripWineForm').reset();tripWinePhotoFile=null;
    $('tripWineImageName').textContent='Ingen bilde valgt';
    $('tripWineImagePreview').src='';$('tripWineImagePreview').hidden=true;
  ...
  $('tripWineForm').onsubmit=async e=>{
    ...
    const wine={id:uid(),createdAt:Date.now(),name:$('tripWineName').value.trim(),vintage:$('tripWineVintage').value.trim(),text:$('tripWineText').value.trim(),image:tripWinePhotoFile?await fileToDataURL(tripWinePhotoFile):''};
    trip.wines=[...(trip.wines||[]),wine];await put('trips',trip);tripWinePhotoFile=null;closeModal('tripWineModal');await renderTripDetail();await render();
  };
