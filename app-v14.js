...
function fillCandidate(x){
  if(!x)return; ['name','producer','country','region','vintage','type','grapes','price','productId'].forEach(k=>{if($(k)&&x[k]!=null&&x[k]!=='')$(k).value=Array.isArray(x[k])?x[k].join(', '):x[k]});
...
function rankCandidate(x,query){
  const wanted=new Set(searchKey(query).split(' ').filter(w=>w.length>1));
  const haystack=searchKey([x.name,x.producer,x.vintage,x.country,x.region,x.grapes].flat().filter(Boolean).join(' '));
  const found=[...wanted].filter(w=>haystack.includes(w)).length;
  ...
  return Math.round(coverage*55+exactName+completeness*3+(Number(x.score)||0)*2);
}
