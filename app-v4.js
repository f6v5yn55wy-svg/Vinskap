...
function labelSearchQueries(text){
  ...
  const useful=lines.filter(s=>!generic.test(s));
  ...
  candidates.push(...useful.slice(0,6));
...
async function handleLabel(file){
  ...
  try{
    ...
    if(!queries.length){showStatus('Jeg klarte ikke å lese nok tekst. Ta et skarpere bilde rett forfra.');return}
