...
async function handleBarcode(file){
  ...
  const {code,url,readerLoaded=true}=decoded; $('scanPreview').src=url;$('scanPreview').hidden=false;
  if(!code){showStatus(readerLoaded?'Ingen strekkode funnet. Hold kameraet nærmere, fyll bildet med koden og unngå gjenskinn.':'Strekkodeleseren ble ikke lastet. Kontroller internettforbindelsen og åpne appen på nytt.');return}
